...
def get_p2p_ars_price(mercado_pago):
    # Returns the lowest price for buying USDT with ARS and MercadoPago as a payment method through Binance's P2P.
    ...
    if mercado_pago:
        ...
        for seller in sellers:
            ...
            for method in tradeMethods:
                if (method["tradeMethodName"] == "Mercadopago"            and 
                    float(seller["adv"]["minSingleTransAmount"]) >= 25000 and 
                    float(seller["adv"]["surplusAmount"]) >= 50.0 ):
    ...
    if mp_sellers:
        ...
        prices = [float(seller["adv"]["price"]) for seller in mp_sellers]
    ...
    # If there are no valid sellers, get the cheapest one without considering mercado pago
    else:
        last_price_usdt = str(sellers[0]["adv"]["price"]) + " (Cash)"
